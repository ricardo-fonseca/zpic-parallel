...
#define CHECK_ERR( err_, msg_ ) { \
    ...
    if ( __local_err != hipSuccess ) { \
        std::cerr << "(*error*) " << (msg_) << std::endl; \
        std::cerr << "(*error*) code: " << __local_err << ", reason: " << hipGetErrorString(__local_err) << std::endl; \
        std::cerr << "(*error*) error state in " << __func__ << "()"; \
        std::cerr << " (" << __FILE__ << ":" << __LINE__ << ")\n"; \
        __local_err = hipDeviceReset(); \
        exit(1); \
    } \
}
...
/**
 * @brief Checks if there are any synchronous or asynchronous errors from hip calls
 * 
 * @note If any errors are found the routine will print out the error messages and exit
 *       the program
 */
#define deviceCheck() { \
    auto err_sync = hipPeekAtLastError(); \
    auto err_async = hipDeviceSynchronize(); \
    if (( err_sync != hipSuccess ) || ( err_async != hipSuccess )) { \
        std::cerr << "(*error*) hip device is on error state at " << __func__ << "()"; \
        std::cerr << " (" << __FILE__ << ":" << __LINE__ << ")\n"; \
        if ( err_sync != hipSuccess ) \
            std::cerr << "(*error*) Sync. error message: " << hipGetErrorString(err_sync) << " (" << err_sync << ") \n"; \
        if ( err_async != hipSuccess ) \
            std::cerr << "(*error*) Async. error message: " << hipGetErrorString(err_async) << " (" << err_async << ") \n"; \
        hipDeviceReset(); \
        exit(1); \
    } \
}
 ...
 * 
 * same as block.thread_rank() in hip cooperative groups, assuming that only 
 * threadIdx.x is used.
 * 
 */
 ...
 * 
 * same as block.num_threads() in hip cooperative groups, assuming that only 
 * threadIdx.x is used.
 * 
 */
 ...
 * 
 * same as grid.thread_rank() in hip cooperative groups, assuming that only 
 ...
 * 
 * same as block.sync() in hip cooperative groups
 * 
 */
...
static size_t shared_mem_size( int id = 0 ) {
    hipDeviceProp_t prop;
    auto err = hipGetDeviceProperties(&prop, id);
    if ( err != hipSuccess ) {
        std::cerr << "(*error*) Unable to query device(" << id << ") properties.\n";
        std::cerr << "(*error*) code: " << err << ", reason: " << hipGetErrorString(err) << "\n";
        err = hipDeviceReset();
        exit(1);
    }
...
template<class T>
static __inline__ __host__ hipError_t set_shmem_size(
  T *entry, size_t shm_size
) {
    if ( shm_size > 49152 ) {
        return hipFuncSetAttribute( (const void *) entry, hipFuncAttributeMaxDynamicSharedMemorySize, shm_size );
    } else {
        return hipSuccess;
    }
}
...
T * malloc( std::size_t const size ) {
    T * buffer;
    auto err = hipMalloc( &buffer, size * sizeof(T) );
    if ( err != hipSuccess ) {
...
void zero( T * const __restrict__ ptr, std::size_t const size ) {
    auto err = hipMemsetAsync( ptr, 0, size * sizeof(T) );
    if ( err != hipSuccess ) {
        std::cerr << "(*error*) Unable to zero device memory." << std::endl;
        std::cerr << "(*error*) code: " << err << ", reason: " << hipGetErrorString(err) << std::endl;
        err = hipDeviceReset();
        exit(1);
    }
}
...
/**
 * @brief hip kernel for setval routine
 * 
 * @tparam T 
 * @param d_data 
 * @param size 
...
void memcpy_tohost( T * const __restrict__ h_out, T const * const __restrict__ d_in, size_t const size) {
    ...
    auto err = hipMemcpy( h_out, d_in, size * sizeof(T), hipMemcpyDeviceToHost );
    if ( err != hipSuccess ) {
...
template< typename T> class Var {
     ...
     */
    Var() {
        auto err = hipMallocManaged( &data, sizeof(T) );
...
T * malloc( std::size_t const size ) {
    ...
    T * buffer;
    auto err = hipHostMalloc( (void **) &buffer, size * sizeof(T) );
    if ( err != hipSuccess ) {
