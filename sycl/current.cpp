 ...
 */
template<>
struct sycl::is_device_copyable<bnd<current::bc::type>> : std::true_type {};
...
Current::Current( uint2 const ntiles, uint2 const nx, float2 const box,
    float const dt, sycl::queue & q ) : q(q), box(box), 
    ...
    dt(dt)
{
    ...
    J = new vec3grid<float3> ( ntiles, nx, gc, q );
...
void Current::process_bc() {
    ...
    const auto d_buffer = J -> d_buffer;
    ...
    if ( bc.x.lower > current::bc::periodic || bc.x.upper > current::bc::periodic ) {
        ...
        q.submit([&](sycl::handler &h) {
            ...
            h.parallel_for( 
                ...
                // Start at x cell 0
                float3 * const __restrict__ tile_J = & d_buffer[ tile_off + x_offset ];
    ...
    if ( bc.y.lower > current::bc::periodic || bc.y.upper > current::bc::periodic ) {
        ...
        q.submit([&](sycl::handler &h) {
            ...
            h.parallel_for( 
                ...
                // Start at y cell 0
                float3 * const __restrict__ tile_J = & d_buffer[ tile_off + y_offset ];
