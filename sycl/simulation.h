...
class Simulation {
    ...
    Simulation( uint2 const ntiles, uint2 const nx, float2 const box, float dt,
        sycl::queue & queue ):
        iter(0), queue(queue), ntiles( ntiles ), nx( nx ), box( box ), dt( dt ), 
        emf( ntiles, nx, box, dt, queue ),
        current( ntiles, nx, box, dt, queue ) {
    }
    ...
    void add_species( Species & s ) {
        ...
        s.initialize( box, ntiles, nx, dt, species_id, queue );
    }
