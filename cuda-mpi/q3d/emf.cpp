...
__global__ __launch_bounds__(opt_yee_block) void yee0( 
    ...
    float2 const dx, const double dt ) {
    ...
    const int ir0 = ( global_tile_off.y + tile_idx.y ) * nx.y;
...
void EMF::advance() {
    ...
    const auto ntiles    = E0.get_ntiles();
    ...
    kernel::yee0 <<< grid, block, shm_size >>> (
        ...
        ntiles, E0.nx, E0.ext_nx,
        E0.offset, E0.get_tile_off(),
        dx, dt
    );
    ...
    for( int m = 1; m < nmodes; m++ ) { 
        ...
        kernel::yeem <<< grid, block, shm_size >>> (
            ...
            ntiles, Em.nx, Em.ext_nx, 
            Em.offset, Em.get_tile_off(),
            dx, dt
        );
    }
...
void EMF::advance( Current & current ) {
    ...
    kernel::yee0J <<< grid, block, shm_size >>> (
        E0.d_buffer, B0.d_buffer, 
        ntiles, E0.nx, E0.ext_nx, E0.offset,
    ...
    for( int m = 1; m < nmodes; m++ ) { 
        ...
        kernel::yeemJ <<< grid, block, shm_size >>> (
            m, Em.d_buffer, Bm.d_buffer, 
            ntiles, Em.nx, Em.ext_nx, Em.offset,
