...
void div_corr_z_scan (
    ...
    const uint2 global_tile_off, 
    float2 const dx, ops::complex<double> * const __restrict__ sendbuf
)
{
    ...
    if ( global_grid_j > 0 ) { // non-axial values
    ...
    if ( block_thread_rank() == 0 ) {
        ...
        sendbuf[ local_nx_y + grid_j ] = divBz;
    }
...
void div_corr_z_sum (
    ...
    cyl_cfloat3 * const __restrict__ B_buffer,
    uint2 const ntiles, uint2 const nx, uint2 const ext_nx,
    ops::complex<double> * const __restrict__ recvbuf
)
{
    int local_nx_y = gridDim.x;
    ...
    auto divBz = recvbuf[ local_nx_y + local_iy ];
    ...
    for( int tx = 0; tx < ntiles.x; tx++ ) {
        ...
        for( int ix = block_thread_rank(); ix < nx.x; ix+= block_num_threads() ) {
            tile_E[ ix + iy * ystride ].z += divEz;
            tile_B[ ix + iy * ystride ].z += divBz;
        }
    }    
}
...
void div_corr_z( cyl3grid<ops::complex<float>>& E, cyl3grid<ops::complex<float>>& B, const float2 dx ) {
    ...
    auto  * __restrict__ sendbuf = device::malloc<ops::complex<double>>( 2 * E.local_nx.y );

    // 1 thread group per line
    ...
    if ( E.part.dims.x > 1 ) {
        auto * __restrict__ recvbuf = device::malloc<ops::complex<double>>( 2 * E.local_nx.y );
