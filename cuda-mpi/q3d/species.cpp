...
int constexpr opt_push_block = 256;
...
int constexpr opt_min_blocks = 4096;
    ...
    cyl_cfloat3 * __restrict__ J, const int stride )
{
    ...
    const auto rcr0 = 1.0f / std::sqrt( fma( t0.x, t0.x, t0.y*t0.y ) );
    const auto rcr1 = 1.0f / std::sqrt( fma( t1.x, t1.x, t1.y*t1.y ) );
    ...
    const auto rrm2 = 1.0f / std::sqrt( fma( xif, xif, yif*yif ) );
 ...
 * @param nx                Current grid tile size (internal)
 * @param ext_nx            Current grid tile size (external, includes guard cells)
 ...
 */
__global__ void __launch_bounds__(opt_move_block) move_deposit_0(
    ...
    const int ir0 = (part.tile_off.y + tile_idx.y) * nx.y;
...
__global__
/**
 * @brief Kernel for depositing current (mode 1). Particle motion is not stored.
 * 
 * @note This version allows the use of multiple blocks per tile. It must be
 *       launched with grid( ntiles.x, ntiles.y, blocks_per_tile )
 *       
 ...
 * @param d_nmove           (out) Number of particles pushed (for performance metrics)
 */
void __launch_bounds__(opt_move_block) deposit_1(
    ParticleData const part,
    cyl_cfloat3 * const __restrict__ current_m1, 
    unsigned int const current_offset, uint2 const nx, uint2 const ext_nx,
    float2 const dt_dx
    ...
    /// @brief [shared] Local copy of current density
    auto * tile_buffer_1 = block::shared_mem< cyl_cfloat3 >();
    ...
    if ( end > begin ) {
        ...
        if ( gridDim.z > 1 ) {
            ...
            for( auto i =  block_thread_rank(); i < 3 * 2 * tile_size; i+= block_num_threads() ) {
                device::atomic_fetch_add( & dev_J1[ i ], shm_J1[ i ] );
            }
...
void Species::initialize( int nmodes_, float2 const box_, uint2 const global_ntiles, uint2 const nx,
    ...
    auto  global_nx = nx * global_ntiles;
    ...
    // Reference maximum number of local particles
    unsigned int max_part = 1.2 * lnx.x * lnx.y * ppc.x * ppc.y * ppc.z;
    ...
    if ( tile_idx.y == 1 ) {
        ...
        case( species::bc::reflecting ) :
            for( int i = block_thread_rank(); i < np; i+= block_num_threads() ) {
                ...
                const auto costh = th[i].x;
                const auto sinth = th[i].y;
                auto       ur  = u[i].x * costh + u[i].y * sinth;
                const auto uth = u[i].y * costh - u[i].x * sinth;

                ur = -ur;
                u[i].x = ur * costh - uth * sinth;
                u[i].y = ur * sinth + uth * costh;
                }
            }
            break;
        default:
...
void Species::advance_mov_window( Current &current ) {
    ...
    if ( moving_window.needs_move( (iter+1) * dt ) ) {
        ...
        if ( particles->parallel.get_coords().x == static_cast<int> (particles->parallel.dims.x ) - 1 ) {
            ...
            range.x = { local_nx.x - 1, local_nx.x - 1 };
            range.y = {              0, local_nx.y - 1 };
 ...
 * @brief Move particles (advance positions), deposit current and shift positions
 * 
 * @note For nmodes = 2, this version will:
 *        1. Deposit current for mode 1
 *        2. Move and deposit current for mode 0
 * 
 * @param current   Electric current density
 * @param shift     Cell shift
 */
void Species::move( Current & current, const int2 shift )
{
    ...
    case 2: {
        // Deposit J1 current
        auto & J1 = current.mode(1);
        size_t shm_size = J1.tile_vol * sizeof( cyl_cfloat3 );
        block::set_shmem_size( kernel::deposit_1, shm_size );
        kernel::deposit_1 <<< grid, block, shm_size >>> ( 
            *particles,
            J1.d_buffer, J1.offset, J1.nx, J1.ext_nx, 
            dt_dx
        );

        // Deposit J0 current and move particles
        shm_size = J0.tile_vol * sizeof( cyl_float3 );
        block::set_shmem_size( kernel::move_deposit_0, shm_size );
        kernel::move_deposit_0 <<< grid, block, shm_size >>> ( 
            *particles,
            J0.d_buffer, J0.offset, J0.nx, J0.ext_nx, 
...
void move(
    ...
    const int ir0 = (part.tile_off.y + tile_idx.y) * part.nx.y;
    ...
    for( int i = block_thread_rank(); i < np; i+= block_num_threads() ) {
        ...
        /// @brief xi + xf
        auto xif = fma( ri, cos_th, xf );
        /// @brief yi + yf
        auto yif = fma( ri, sin_th, yf );
...
__global__ void charge_norm(
    ...
    int ir0 = (par_tile_off.y + tile_idx.y ) * nx.y;
...
void Species::deposit_charge0( grid<float> & charge0 ) const {
    ...
    kernel::charge_norm <<< grid, block >>> (
        particles -> tile_off, charge0.get_ntiles(), charge0.d_buffer, charge0.offset, 
...
void Species::deposit_charge( const unsigned m, grid<ops::complex<float>> &charge ) const {
    ...
    kernel::charge_norm <<< grid, block >>> (
        particles -> tile_off, charge.get_ntiles(), charge.d_buffer, charge.offset, 
...
void Species::save_charge( const unsigned m ) const {
    ...
    if ( m == 0 ) {
        ...
        // High-order mode
        grid<ops::complex<float>> charge( particles -> global_ntiles, particles -> nx, gc, particles -> parallel );
...
void Species::save() const {
    ...
    if ( global > 0 ) {
        ...
        if ( local > 0 ) {
            ...
            // Allocate device and host buffers for gathering particle data
    ...
    int const size ) const
{
    ...
    // Save file    // Add contributions from all parallel nodes to root node
