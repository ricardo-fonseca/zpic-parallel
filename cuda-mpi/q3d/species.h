...
public:
    ...
    void set_bc( species::bc_type new_bc ) {
        ...
        // Set periodic flags on particles object
        if ( particles ) {
            particles -> set_periodic( make_int2( bc.x.lower == species::bc::periodic, 0 ));
        }
    }
    ...
    virtual int set_moving_window() { 
        if ( iter == 0 ) {
            ...
            particles -> set_periodic( make_int2(0,0) );
            return 0;
        } else {
     ...
     */
    void deposit_charge( const unsigned m, grid<ops::complex<float>> &charge ) const;
    ...
    double get_energy() const {
        ...
        return d_energy.get() * m_q * dx.x * dx.y * 2 * M_PI;
    }
