    ...
    ParticleData const part )
{
    ...
    if (( ri0 < nx.x ) && ( ri1 >= 0 ) &&
        ...
        const int shiftr = ( part.tile_off.y + tile_idx.y ) * nx.y;
            ...
            for( unsigned ir = 0; ir < ppc.y; ir++ ) {
                for( unsigned iz = 0; iz < ppc.x; iz++) {
                    ...
                    for( int grid_idx = block_thread_rank(); grid_idx < grid_vol; grid_idx += block_num_threads() ) {
                        ...
                        auto r = ( shiftr + cell.y ) + double( pos.y );
                        ...
                        q [ part_idx ] = r * ( (shiftr + cell.y == 0) ? α * qnorm : qnorm );
...
void uniform_np( 
    ...
    if ( tile_id < part.ntiles.x * part.ntiles.y ) {
        ...
        if (( ri0 < nx.x ) && ( ri1 >= 0 ) &&
            ...
            int const row = (ri1-ri0+1);
            if ( ( part.tile_off.y + tile_idx.y ) * nx.y + rj0 > 0 ) {
    ...
    ParticleData const part )
{
    ...
    if (( ri0 < nx.x ) && ( ri1 >= 0 ) &&
        ...
        const int shiftz = ( part.tile_off.x + tile_idx.x ) * nx.x;
        const int shiftr = ( part.tile_off.y + tile_idx.y ) * nx.y;
