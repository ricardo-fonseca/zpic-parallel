...
void current_norm_0(
    ...
    auto * __restrict__ current = &  d_current[ tile_off ];
    ...
    for( int idx = block_thread_rank(); idx < range_z * range_r; idx += block_num_threads() ) {
        ...
        float rl   = std::abs( j0 + j - 0.5f ) * dr;
        
        float norm_r  = ( j0 + j == 0 )? 0 : 1.0f / rc;
...
void current_norm_m(
    unsigned const m,
    cyl3< ops::complex<float> > * const __restrict__ d_current,
    uint2 const ntiles, uint2 const nx, uint2 const ext_nx,
    float2 const dx, double dt,
    uint2 const  global_tile_off
) {
    ...
    for( int idx = block_thread_rank(); idx < range_z * range_r; idx += block_num_threads() ) {
        ...
        float norm_r  = ( j0 + j == 0 )? 0 : (2 * dr_dt) / rc;
...
void Current::process_bc() {
    ...
    if ( bc.x.lower > current::bc::periodic || bc.x.upper > current::bc::periodic ) {
        dim3 grid( 2, J0.get_ntiles().y );

        kernel::current_bcz <<< grid, block >>> ( 
            J0.d_buffer, 
            J0.get_ntiles(), J0.nx, J0.ext_nx, J0.gc, 
            bc
        );
        ...
        for( int m = 1; m < nmodes; m++ ) {
            ...
            kernel::current_bcz <<< grid, block >>> ( 
                Jm.d_buffer, 
                Jm.get_ntiles(), Jm.nx, Jm.ext_nx, Jm.gc, 
                bc
            );
        }
    }
...
void Current::normalize() {
    ...
    dim3 grid( J0.get_ntiles().x, J0.get_ntiles().y );
    dim3 block( 128 );

    // Normalize mode 0
    kernel::current_norm_0 <<< grid, block >>> (
        & J0.d_buffer[ J0.offset ], 
        J0.get_ntiles(), J0.nx, J0.ext_nx, 
        dx, dt, J0.get_tile_off()
    );
    ...
    for( int m = 1; m < nmodes; m++ ) {
        ...
        kernel::current_norm_m <<< grid, block >>> (
            ...
            Jm.get_ntiles(), Jm.nx, Jm.ext_nx, 
            dx, dt, Jm.get_tile_off()
        );
    }
}
