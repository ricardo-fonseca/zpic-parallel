 ...
 * @param periodic      (in) Periodic boundaries
 */
void bnd_check( 
    ParticleData part, ParticleSortData sort, const int2 periodic )
{
    ...
    for( auto tid = 0; tid < ntiles.y * ntiles.x; tid ++ ) {
        ...
        for( int i =0; i < 9; ++i ) {
            ...
            int target_tid = part::tid_coords( target, ntiles, periodic );
 ...
 */
void copy_out( 
    ParticleData part, ParticleData tmp, const ParticleSortData sort, int2 const periodic )
{
    const int2 ntiles = make_int2( part.ntiles.x, part.ntiles.y );
    const int2 lim = make_int2( part.nx.x, part.nx.y );
