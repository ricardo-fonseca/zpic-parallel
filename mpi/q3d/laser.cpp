...
void div_corr_z( cyl3grid<std::complex<float>>& E, cyl3grid<std::complex<float>>& B, const float2 dx ) {
    ...
    auto * __restrict__ sendbuf = memory::malloc<std::complex<double>>( 2 * local_nx.y );

    /// @brief imaginary unit
    constexpr std::complex<float> I{0,1};

    // Start away from axis
    int start = ( global_tile_off.y == 0 ) ? 1 : 0;

    #pragma omp parallel for
    for( unsigned int grid_j = start; grid_j < local_nx.y; grid_j ++ ) {

        std::complex<double> divEz = 0;
        std::complex<double> divBz = 0;

        ///@brief y tile index
        const int ty = grid_j / nx.y;
        
        ///@brief j coordinate inside tile
        const int j = grid_j - ty * nx.y;
        
        ///@brief global j coordinate
        const int global_grid_j = grid_j + global_tile_off.y * nx.y;

        ///@brief r/Δr at center of cell
        const float rc  = global_grid_j;
        ///@brief r/Δr at center of lower (j-1) cell
        const float rcm = global_grid_j - 1;
        ///@brief r/Δr at upper edge of cell
        const float rp = global_grid_j + 0.5;
        ///@brief r/Δr at lower edge of cell
        const float rm = global_grid_j - 0.5;
        ...
        sendbuf[              grid_j ] = divEz;
        sendbuf[ local_nx.y + grid_j ] = divBz;
    }

    // If there is a parallel partition along x, add in contribution from nodes to the right
    if ( E.part.dims.x > 1 ) {
        auto * __restrict__ recvbuf = memory::malloc<std::complex<double>>( 2 * local_nx.y );
        ...
        MPI_Exscan( sendbuf, recvbuf, 2 * local_nx.y, MPI_C_DOUBLE_COMPLEX, MPI_SUM, newcomm );
        ...
        if ( key > 0 ) {
            ...
            for( int local_iy = 0; local_iy < local_nx.y; local_iy ++ ) {
                ...
                auto divEz = recvbuf[              local_iy ];
                auto divBz = recvbuf[ local_nx.y + local_iy ];
                ...
                for( int tx = 0; tx < ntiles.x; tx++ ) {
                    ...
                    for( int ix = 0; ix < nx.x; ix++ ) {
                        ...
                        tile_E[ ix + iy * jstride].z += divEz;
                        tile_B[ ix + iy * jstride].z += divBz;
                    }
                }
            }
        }
...
int Laser::PlaneWave::launch( cyl3grid<std::complex<float>>& E, cyl3grid<std::complex<float>>& B, float2 box ) {
    ...
    mpi::cout << "Tile offset: " << global_tile_off << '\n';
    ...
    for( unsigned int tid = 0; tid < ntiles.y * ntiles.x; tid++ ) {
        ...
        const int i0 = ( global_tile_off.x + tx ) * nx.x;
...
int Laser::Gaussian::launch( cyl3grid<std::complex<float>>& E, cyl3grid<std::complex<float>>& B, float2 const box ) {
    ...
    const auto global_tile_off  = E.get_tile_off();
    ...
    for( unsigned int tid = 0; tid < ntiles.y * ntiles.x; tid++ ) {
        ...
        const int i0 = ( global_tile_off.x + tx ) * nx.x;
        const int j0 = ( global_tile_off.y + ty ) * nx.y;
