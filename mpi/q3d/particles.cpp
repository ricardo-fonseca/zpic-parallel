...
uint32_t update_tile_info( 
    ...
    // Exclusive scan
    uint32_t total = 0;
    for( unsigned i = 0; i < ntiles_all; i++ ) {
...
void Particles::isend_msg( Particles &tmp, ParticleSort &sort, ParticleMessage &send ) {
    ...
    auto * const __restrict__ ix = &tmp.ix[ tile_off ];
    auto * const __restrict__ x  = &tmp.x[ tile_off ];
    auto * const __restrict__ u  = &tmp.u[ tile_off ];
    auto * const __restrict__ q  = &tmp.q[ tile_off ];
    auto * const __restrict__ θ  = &tmp.θ[ tile_off ];
    ...
    for( int dir = 0; dir < 9; dir++) {
        if ( sort.send.msg_np[dir] > 0 ) {
            ...
            nbytes = np * sizeof(float);
            memcpy( &buffer[pos],  &q[ off[dir] ], nbytes);
            pos += nbytes;

            nbytes = np * sizeof(float2);
            memcpy( &buffer[pos],  &θ[ off[dir] ], nbytes);
            //pos += nbytes;  // unecessary
...
void Particles::unpack_msg( ParticleSort &sort, ParticleMessage &recv ) {
    ...
    for( auto dir = 0; dir < 9; dir++ ) {
        if ( dir != 4 ) {
            ...
            int2   * ix_buffer = (int2 *)   & msg_buffer[ msg_off ]; msg_off += msg_np * sizeof(int2);
            float2 * x_buffer  = (float2 *) & msg_buffer[ msg_off ]; msg_off += msg_np * sizeof(float2);
            float3 * u_buffer  = (float3 *) & msg_buffer[ msg_off ]; msg_off += msg_np * sizeof(float3);
            float  * q_buffer  = (float *)  & msg_buffer[ msg_off ]; msg_off += msg_np * sizeof(float);
            float2 * θ_buffer  = (float2 *) & msg_buffer[ msg_off ];

            ///@brief number of particles unpacked from this message
            int np_unpack = 0;

            for( unsigned k = 0; k < msg_ntiles(dir); k++ ) {
                ...
                if ( recv_np > 0 ) {
                    ...
                    int target_tid = k * tile_stride(dir) + tile_offset(dir);

                    // destination buffers (in main particle buffer)
                    int tgt_offset  = offset[ target_tid ] + 
                        omp::atomic_fetch_add( &np[ target_tid ], recv_np );
                    ...
                    float  * __restrict__ const dst_q  = &q [ tgt_offset ];
                    float2 * __restrict__ const dst_θ  = &θ [ tgt_offset ];
                    ...
                    float *  __restrict__ const src_q  = & q_buffer[ np_unpack ];
                    float2 * __restrict__ const src_θ  = & θ_buffer[ np_unpack ];
