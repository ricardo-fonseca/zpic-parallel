...
static_assert( sizeof( unsigned int ) == 4, "RANDOM module only works if `unsigned int` is 32 bits" );
 ...
 * 
 * @note This is meant to be called by multiple threads supplying different
 *       `global_id` values allowing the PRNG to be used in parallel
 *       environment
 * 
 * @param global_id Unique global thread id 
 * @param seed      Seed value
 * @param state     PRNG state variable
 */
__host__ __device__ __inline__
void rand_init( const int global_id, const uint2 seed, uint2 & state ) {
 ...
 */
__host__ __device__ __inline__
void rand_init( const int global_id, const uint2 seed, uint2 & state, double & norm ) {
